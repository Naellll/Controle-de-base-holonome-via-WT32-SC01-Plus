...
// DECLARATIONS
LGFX lcd;
int16_t x, y, cmpt = 0, x_fleche = 0, y_fleche = 0;
float distance;
...
void dessin_ecran(){
  ...
  lcd.drawRect(-1, 320, 322, 115, TFT_BLACK);
  lcd.drawRect(-1, 321, 322, 116, TFT_BLACK);
  lcd.drawRect(-1, 435, 322, 50, TFT_BLACK);
  ...
  lcd.drawLine(160, 20, 160, 300, TFT_BLACK);  lcd.drawLine(161, 20, 161, 300, TFT_BLACK);
  lcd.drawLine(20, 160, 300, 160, TFT_BLACK);  lcd.drawLine(20, 161, 300, 161, TFT_BLACK);
  lcd.drawCircle(160, 160, 22, TFT_BLACK);     lcd.drawCircle(160, 160, 23, TFT_BLACK);
  lcd.fillCircle(160, 160, 21, lcd.color565(118, 212, 255));

  // Informations de connexion
  lcd.setTextColor(TFT_BLACK, lcd.color565(118, 212, 255));
  lcd.drawString("Connexion :", 5, 323, 2);
  lcd.drawString("WiFi :", 5, 342, 1);
  lcd.drawString(ssid, 48, 342, 1);
  lcd.drawString(("IP   : " + WiFi.localIP().toString()), 5, 352, 1);
  lcd.drawString(("Port : " + String(port)), 5, 362, 1);

  // Credits
  lcd.drawString("- Conception par :", 5, 440, 1);
  lcd.drawString("Nathanael DEBREILLY, Eliez LE HUNSEC, Ilan CHANY", 5, 450, 1);
  lcd.drawString("- Professeurs referents :", 5, 460, 1);
  lcd.drawString("Stephane BOCHARD, Philippe GICQUEL", 5, 470, 1);
...
void loop() {
  ...
  if (cmpt >= 10 && client && client.connected()) {
    cmpt = 0;
    if (distance <= 22){
      ...
      commande.ConsigneBase.VitesseX = int((450*(x_fleche-160))/160);
      commande.ConsigneBase.VitesseY = int((450*(160-y_fleche))/160);
    }
