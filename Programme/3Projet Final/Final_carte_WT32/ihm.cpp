...
// Logo ENSIBS
const uint16_t COULEUR_FOND = lcd.color565(118, 212, 255);
...
// Effacer l'ancien trait de direction
void effacerAppui() {
  lcd.drawLine(161, 161, x_fleche + 1, y_fleche + 1, COULEUR_FOND);
  lcd.drawLine(160, 160, x_fleche, y_fleche, COULEUR_FOND);
  lcd.drawLine(159, 160, x_fleche - 1, y_fleche, COULEUR_FOND);
  lcd.drawLine(161, 160, x_fleche + 1, y_fleche, COULEUR_FOND);
  lcd.fillCircle(x_fleche, y_fleche, 5, COULEUR_FOND);
}


// Gestion du tactile en fonction de la position de l'appui
void gestionTactile(int xp, int yp) {

  if (yp <= 300 && yp >= 20 && xp >= 20 && xp <= 300){

    distance = sqrt(pow(xp - 160, 2) + pow(yp - 160, 2));
    effacerAppui();
...
// Affichage attente de wifi
void afficherChargement(int etape) {

  lcd.setTextColor(COULEUR_FOND, TFT_BLACK);
  lcd.drawString("En attente du reseau wifi :" , 20, 120, 4);
  lcd.drawString(ssid, 20, 150, 4);
  lcd.fillCircle(160, 240, 40, COULEUR_FOND); // Efface zone

  for (int i = 0; i < 8; i++) {
    int angle = i * 45;
    int x = 160 + 30 * cos(radians(angle));
    int y = 240 + 30 * sin(radians(angle));
    uint16_t couleur = (i == etape % 8) ? TFT_RED : TFT_BLACK;
    lcd.fillCircle(x, y, 4, couleur);
  }
  if (etape == 9) etape = 0;
}


// Fonction de transition vers l'IHM avec logo ENSIBS
void transitionVersIHM() {

  for (int i = 0; i <= 255; i += 5) {
    uint16_t gris = lcd.color565(i, i, i);
    lcd.fillRect(0, 0, 320, 480, gris);
    delay(30);
  }
}
...
// Jauge horizontale
void afficherJaugeHorizontale(int positionX, int positionY, int largeur, int hauteur, int valeur, int valeurMin, int valeurMax) {

  int valeurAbsolue = abs(valeur);

  if (valeurAbsolue < valeurMin) valeurAbsolue = valeurMin;
  else if (valeurAbsolue > valeurMax) valeurAbsolue = valeurMax;

  int plage = valeurMax - valeurMin;
  int valeurRelative = valeurAbsolue - valeurMin;

  float ratio = (float)valeurRelative / plage;
  int largeurRemplie = ratio * largeur;

  uint8_t rouge = ratio * 255;
  uint8_t vert = 255 - rouge;

  lcd.fillRect(positionX, positionY, largeur, hauteur, TFT_WHITE);
  lcd.fillRect(positionX, positionY, largeurRemplie, hauteur, lcd.color565(rouge, vert, 0));
  lcd.drawRect(positionX, positionY, largeur, hauteur, TFT_BLACK);
}


// Cadres de bas d'ecran
void dessinCadres() {
  lcd.drawRect(-1, 320, 322, 90, TFT_BLACK);
  lcd.drawRect(-1, 321, 322, 90, TFT_BLACK);
  lcd.drawRect(-1, 320, 161, 90, TFT_BLACK);
  lcd.drawRect(-1, 321, 162, 90, TFT_BLACK);
}


// Croix du joystick virtuel
void dessinCroixJoystick() {
  lcd.drawLine(140, 20, 180, 20, TFT_BLACK);   lcd.drawLine(140, 21, 180, 21, TFT_BLACK);
  lcd.drawLine(140, 300, 180, 300, TFT_BLACK); lcd.drawLine(140, 301, 180, 301, TFT_BLACK);
  lcd.drawLine(20, 140, 20, 180, TFT_BLACK);   lcd.drawLine(21, 140, 21, 180, TFT_BLACK);
  lcd.drawLine(300, 140, 300, 180, TFT_BLACK); lcd.drawLine(301, 140, 301, 180, TFT_BLACK);
  lcd.drawLine(160, 20, 160, 138, TFT_BLACK);  lcd.drawLine(161, 20, 161, 138, TFT_BLACK);
  lcd.drawLine(160, 182, 160, 300, TFT_BLACK); lcd.drawLine(161, 182, 161, 300, TFT_BLACK);
  lcd.drawLine(20, 160, 138, 160, TFT_BLACK);  lcd.drawLine(20, 161, 138, 161, TFT_BLACK);
  lcd.drawLine(182, 160, 300, 160, TFT_BLACK);  lcd.drawLine(182, 161, 300, 161, TFT_BLACK);
  lcd.drawCircle(160, 160, 22, TFT_BLACK);     lcd.drawCircle(160, 160, 23, TFT_BLACK);
  lcd.fillCircle(160, 160, 21, COULEUR_FOND);
}


// Informations de connexion
void dessinInformationConnexion() {
  lcd.setTextColor(TFT_BLACK, COULEUR_FOND);
  lcd.drawString("Connexion :", 5, 323, 2);
  lcd.drawString("WiFi :", 5, 345, 1);
  lcd.drawString(ssid, 48, 345, 1);
  lcd.drawString(("IP   : " + WiFi.localIP().toString()), 5, 360, 1);
  lcd.drawString(("Port : " + String(port)), 5, 375, 1);
  lcd.drawString( "Base : ", 5, 390, 1);
  if (serveur.hasClient()){
    lcd.setTextColor(TFT_GREEN, COULEUR_FOND);
    lcd.drawString("Connectee", 48, 390, 1);
  }
  else{
    lcd.setTextColor(TFT_RED, COULEUR_FOND);
    lcd.drawString("Deconnectee", 48, 390, 1);
  }
  lcd.setTextColor(TFT_BLACK, COULEUR_FOND);
}
...
void dessinInformationVitesse() {
  lcd.drawString("Vitesse :", 166, 323, 2);
  lcd.drawString("Vitesse en X :", 166, 345, 1);
  lcd.fillRect(250, 345, 40, 10, COULEUR_FOND);
  lcd.drawString("Vitesse en Y :", 166, 375, 1);
  lcd.fillRect(250, 375, 40, 10, COULEUR_FOND);
  if (x_fleche == 0 && y_fleche == 0){
    lcd.drawString(String(x_fleche), 255, 345, 1);
    afficherJaugeHorizontale(170, 360, 140, 10, 0, 22, VITESSE_MAX_ROBOT);
    lcd.drawString(String(y_fleche), 255, 375, 1);
    afficherJaugeHorizontale(170, 390, 140, 10, 0, 22, VITESSE_MAX_ROBOT);
  }
  else{
    lcd.drawString(String(int((VITESSE_MAX_ROBOT*(x_fleche-160))/140)), 255, 345, 1);
    afficherJaugeHorizontale(170, 360, 140, 10, int((VITESSE_MAX_ROBOT*(x_fleche-160))/140), 22, VITESSE_MAX_ROBOT);
    lcd.drawString(String(int((VITESSE_MAX_ROBOT*(160-y_fleche))/140)), 255, 375, 1);
    afficherJaugeHorizontale(170, 390, 140, 10, int((VITESSE_MAX_ROBOT*(160-y_fleche))/140), 22, VITESSE_MAX_ROBOT);
  }
}
...
// Credits
void dessinCredit() {
  lcd.drawString("- Conception par :", 5, 415, 2);
  lcd.drawString("Nathanael DEBREILLY, Eliez LE HUNSEC, Ilan CHANY", 5, 435, 1);
  lcd.drawString("- Professeurs referents :", 5, 445, 2);
  lcd.drawString("Stephane BOCHARD, Philippe GICQUEL", 5, 465, 1);
  lcd.pushImage(235, 5, 80, 32, image_data);
}
...
void dessinAppui() {
  if (x_fleche != 0 && y_fleche != 0) {
    lcd.drawLine(160 + 1, 160 + 1, x_fleche + 1, y_fleche + 1, lcd.color565(60, 90, 150));
    lcd.drawLine(160, 160, x_fleche, y_fleche, lcd.color565(80, 120, 200));
    lcd.drawLine(159, 160, x_fleche - 1, y_fleche, lcd.color565(80, 120, 200));
    lcd.drawLine(161, 160, x_fleche + 1, y_fleche, lcd.color565(80, 120, 200));
    lcd.fillCircle(x_fleche, y_fleche, 5, lcd.color565(80, 120, 200));
    lcd.drawCircle(160, 160, 22, TFT_BLACK);     lcd.drawCircle(160, 160, 23, TFT_BLACK);
    lcd.fillCircle(160, 160, 21, COULEUR_FOND);
  }
}
...
void affichageIHM() {
  ...
  dessinCadres();
  dessinCroixJoystick();
  dessinInformationConnexion();
  dessinInformationVitesse();
  dessinCredit();
  dessinAppui();
}
...
void actualiserIHM() {
  ...
  // Actualisation connexion
  lcd.fillRect(47, 389, 80, 10, COULEUR_FOND);
  if (serveur.hasClient()){
    lcd.setTextColor(TFT_GREEN, COULEUR_FOND);
    lcd.drawString("Connectee", 48, 390, 1);
  }
  ...
  lcd.fillRect(254, 345, 40, 10, COULEUR_FOND);
  lcd.fillRect(254, 375, 40, 10, COULEUR_FOND);
  if (x_fleche == 0 && y_fleche == 0){
    lcd.drawString(String(x_fleche), 255, 345, 1);
    afficherJaugeHorizontale(170, 360, 140, 10, 0, 22, VITESSE_MAX_ROBOT);
    lcd.drawString(String(y_fleche), 255, 375, 1);
